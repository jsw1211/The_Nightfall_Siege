...
{
    if (QuestStage == EQuestStage::ClearDungeon) QuestStage = EQuestStage::SpendSkillPoint;
}

void ABasePlayerState::NotifySkillPointSpent()
{
    if (QuestStage == EQuestStage::SpendSkillPoint) QuestStage = EQuestStage::CollectPrism;
}
...
void ABasePlayerState::NotifyPrismCollected()
{
    ...
    ++ClearedDungeonCount;
    QuestStage = ClearedDungeonCount >= 3 ? EQuestStage::FindBossPortal : EQuestStage::FindDungeonPortal;
}
...
FText ABasePlayerState::GetQuestObjectiveText() const
{
    ...
    case EQuestStage::SpendSkillPoint: return FText::FromString(TEXT("획득한 스킬 포인트로 스킬을 강화하세요."));
